"use client";
// Background interaktif global — canvas grid yang warp ke arah cursor +
// ripple tiap klik (dipasang di AppShell, lihat components/layout/AppShell.tsx,
// jadi aktif di SEMUA halaman kerja: dashboard, generate, history, dst).
//
// Diadaptasi dari komponen kinetic-grid pihak ketiga, 2 perubahan dari versi
// asli supaya nyatu dgn desain Deera Studio yang sudah ada (globals.css):
// 1. Warna default diganti dari biru generic -> emas brand Deera (gold/
//    gold-soft), "monochrome" tetap disediakan sbg alternatif netral.
// 2. Canvas TIDAK fillRect bg solid lagi — dibiarkan transparan supaya
//    radial-gradient glow (amber+violet) & grain di body (globals.css)
//    tetap kelihatan di baliknya. Grid ini jadi LAPISAN TAMBAHAN di atas
//    depth yang sudah ada, bukan pengganti flat solid color.
...
export default function KineticGrid({
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      const theme = {
        default: {
          // Emas brand Deera (--color-gold / --color-gold-soft di globals.css)
          lineActive: { r: 217, g: 162, b: 78, a: 0.9 },
          nodeActive: { r: 232, g: 187, b: 120, a: 1.0 },
          glow: "217,162,78",
          ripple: "232,187,120",
        },
        monochrome: {
      ...
      // TIDAK fillRect bg solid (beda dari versi asli) — canvas dibiarkan
      // transparan supaya radial-gradient + grain body (globals.css) tetap
      // kelihatan di baliknya.
