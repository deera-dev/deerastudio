...
// dgn batas KONKRET & terukur ("ends AT THE NECK", "no acceptable amount of
// shoulder coverage") drpd istilah relatif "shoulder-length" yang ternyata
// masih dibaca longgar oleh model AI. CATATAN yang SAMA masih berlaku: kalau
// ini juga belum cukup, akar masalahnya kemungkinan besar tetap di foto
// referensi "Pose Belakang" itu sendiri (app/poses/page.tsx) — pertimbangkan
// upload ulang foto itu dgn model yang SUDAH berjilbab pendek dari awal.
//
...
function buildPrompt(input: NanoBananaGenerateInput): string {
  ...
  return [
    ...
    "",
    input.isBackView
      ? "Before finalizing, verify internally: is this clearly the same model? Does the garment match the PRODUCT REFERENCE (flat-lay) images in color, pattern, embroidery, silhouette, and proportions — as if it is the same physical product, not a redesign? Does it look like a real professional photograph? Does the hijab end AT THE NECK with ZERO fabric on the shoulders or back, and is the ENTIRE back of the garment (including both shoulders) visible — if the hijab in your draft touches the shoulders or covers any garment detail even slightly, redo it shorter (neck-length only) before finalizing, even though the reference photos show a longer hijab. If any product detail conflicts with what the model is wearing in the MODEL REFERENCE images, always prioritize the PRODUCT REFERENCE images."
